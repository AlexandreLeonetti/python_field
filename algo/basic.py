...
def indicator(data: pd.DataFrame, period: int):
      #print(data)
      #data["highDon"] = data["High"] #data["High"].rolling(period).max()
      #data["lowerDon"] = data["Low"]#.rolling(period).min()
      #data["midDon"] = (data["highDon"] + data["lowerDon"]) / 2
      #data["yes"] = 9 # this adds a column named "yes" each elements of value 9
      return data

#downloading the correct data from yahoo finance
ticker = "LTC-USD"
yfObj = yf.Ticker(ticker) #returns data as panda dataframe.
data = yfObj.history(start="2023-10-01", end="2023-10-15").drop(
...
def strategy(data: pd.DataFrame, period: int=10, shorts: bool=True):
  ...
  data["position"] = np.where(data["Close"]>data["Open"],1, data["position"])#data["position"]) #,data["position"])
